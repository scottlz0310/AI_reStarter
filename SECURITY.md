...
We release patches for security vulnerabilities. Which versions are eligible for receiving such patches depends on the version compatibility:
...
- Type of issue (e.g., buffer overflow, memory safety issue, injection, etc.)
...
- **Memory Safety**: Built on Rust to prevent common memory vulnerabilities (buffer overflows, etc.)
- **Dependency Scanning**: Automated vulnerability scanning with `cargo audit` and Dependabot
- **Static Analysis**: Continuous code quality checks with `clippy`
- **Secret Scanning**: Automated checks for committed secrets
