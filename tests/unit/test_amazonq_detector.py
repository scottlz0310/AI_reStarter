...
class TestAmazonQDetector:
    ...
    def test_detect_state_run_button_found(self, mock_cvtColor, mock_minMaxLoc, mock_matchTemplate, detector):
        """▶RUNボタン検出成功のテスト"""
        # テンプレートを手動で追加
        ...
        # モックの設定
        mock_cvtColor.return_value = np.ones((400, 600), dtype=np.uint8)
        mock_matchTemplate.return_value = np.ones((350, 550), dtype=np.float32)
        mock_minMaxLoc.return_value = (0.1, 0.9, (50, 100), (100, 200))
    ...
    @patch('src.plugins.amazonq_detector.cv2.minMaxLoc')
    @patch('src.plugins.amazonq_detector.cv2.cvtColor')
    def test_detect_state_run_button_not_found(self, mock_cvtColor, mock_minMaxLoc, mock_matchTemplate, detector):
        """▶RUNボタン検出失敗のテスト"""
        # テンプレートを手動で追加
        template = np.ones((50, 50), dtype=np.uint8)
        detector.run_button_templates["run"] = template
        ...
        mock_cvtColor.return_value = np.ones((400, 600), dtype=np.uint8)
        mock_matchTemplate.return_value = np.ones((350, 550), dtype=np.float32)
        mock_minMaxLoc.return_value = (0.1, 0.7, (50, 100), (100, 200))  # 閾値0.8以下
    ...
    @patch('src.plugins.amazonq_detector.cv2.minMaxLoc')
    @patch('src.plugins.amazonq_detector.cv2.cvtColor')
    def test_logging_on_detection_success(self, mock_cvtColor, mock_minMaxLoc, mock_matchTemplate, mock_logger, detector):
        """検出成功時のログ出力テスト"""
        # テンプレートを手動で追加
        template = np.ones((50, 50), dtype=np.uint8)
        detector.run_button_templates["run"] = template
