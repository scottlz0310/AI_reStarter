...
class TestImageProcessor:
    ...
    def test_detect_error_found(
        ...
        result = processor.detect_error(
            sample_screenshot, error_templates, threshold=0.8
        )
    ...
    @patch("src.utils.image_processing.cv2.matchTemplate")
    @patch("src.utils.image_processing.cv2.minMaxLoc")
    def test_detect_error_not_found(
        self,
        mock_min_max_loc,
        mock_match_template,
        processor,
    ...
    @patch("src.utils.image_processing.cv2.matchTemplate")
    def test_detect_error_exception(
        self, mock_match_template, processor, sample_screenshot, error_templates
    ):
        """エラー検出時の例外処理テスト"""
    ...
    @patch("src.utils.image_processing.cv2.matchTemplate")
    @patch("src.utils.image_processing.cv2.minMaxLoc")
    def test_find_template_position_found(
        self,
        mock_min_max_loc,
        mock_match_template,
        processor,
        ...
        position = processor.find_template_position(
            sample_screenshot, sample_template, threshold=0.7
        )
    ...
    @patch("src.utils.image_processing.cv2.matchTemplate")
    @patch("src.utils.image_processing.cv2.minMaxLoc")
    def test_find_template_position_not_found(
        self,
        mock_min_max_loc,
        mock_match_template,
        processor,
    ...
    def test_find_template_position_template_too_large(
        self, processor, sample_screenshot
    ):
    ...
    @patch("src.utils.image_processing.cv2.matchTemplate")
    def test_find_template_position_exception(
        self, mock_match_template, processor, sample_screenshot, sample_template
    ):
        """テンプレート位置検出時の例外処理テスト"""
    ...
    def test_save_template_success(
        self, mock_makedirs, mock_imwrite, processor, sample_template
    ):
        """テンプレート保存成功のテスト"""
    ...
    def test_load_templates_success(
        self, mock_imread, mock_listdir, mock_exists, processor
    ):
        """テンプレート読み込み成功のテスト"""
    ...
    @patch("src.utils.image_processing.os.path.exists")
    @patch("src.utils.image_processing.os.makedirs")
    def test_load_templates_directory_not_exists(
        self, mock_makedirs, mock_exists, processor
    ):
    ...
    @patch("src.utils.image_processing.cv2.matchTemplate")
    @patch("src.utils.image_processing.cv2.minMaxLoc")
    def test_logging_on_error_detection(
        self,
        mock_min_max_loc,
        mock_match_template,
        mock_logger,
    ...
    def test_detect_error_with_monitor_region(
        self, processor, sample_screenshot, error_templates
    ):
        """監視領域指定でのエラー検出テスト"""
        ...
        with (
            ...
            result = processor.detect_error(
                sample_screenshot, error_templates, monitor_region=monitor_region
            )
