...
class TestImageProcessor:
    ...
    def test_detect_error_found(self, mock_minMaxLoc, mock_matchTemplate, processor, sample_screenshot, error_templates):
        """エラー検出成功のテスト"""
        # モックの設定
        mock_matchTemplate.return_value = np.array([[0.9]])
        mock_minMaxLoc.return_value = (0.1, 0.9, (10, 10), (20, 20))
    ...
    @patch('src.utils.image_processing.cv2.matchTemplate')
    @patch('src.utils.image_processing.cv2.minMaxLoc')
    def test_detect_error_not_found(self, mock_minMaxLoc, mock_matchTemplate, processor, sample_screenshot, error_templates):
        """エラー検出失敗のテスト"""
        # モックの設定（信頼度が閾値以下）
        mock_matchTemplate.return_value = np.array([[0.5]])
        mock_minMaxLoc.return_value = (0.1, 0.5, (10, 10), (20, 20))
    ...
    @patch('src.utils.image_processing.cv2.matchTemplate')
    def test_detect_error_exception(self, mock_matchTemplate, processor, sample_screenshot, error_templates):
        """エラー検出時の例外処理テスト"""
        mock_matchTemplate.side_effect = Exception("Template matching failed")
    ...
    @patch('src.utils.image_processing.cv2.matchTemplate')
    @patch('src.utils.image_processing.cv2.minMaxLoc')
    def test_find_template_position_found(self, mock_minMaxLoc, mock_matchTemplate, processor, sample_screenshot, sample_template):
        """テンプレート位置検出成功のテスト"""
        # モックの設定
        mock_matchTemplate.return_value = np.array([[0.8]])
        mock_minMaxLoc.return_value = (0.1, 0.8, (10, 10), (20, 20))
    ...
    @patch('src.utils.image_processing.cv2.matchTemplate')
    @patch('src.utils.image_processing.cv2.minMaxLoc')
    def test_find_template_position_not_found(self, mock_minMaxLoc, mock_matchTemplate, processor, sample_screenshot, sample_template):
        """テンプレート位置検出失敗のテスト"""
        # モックの設定（信頼度が閾値以下）
        mock_matchTemplate.return_value = np.array([[0.5]])
        mock_minMaxLoc.return_value = (0.1, 0.5, (10, 10), (20, 20))
    ...
    @patch('src.utils.image_processing.cv2.matchTemplate')
    def test_find_template_position_exception(self, mock_matchTemplate, processor, sample_screenshot, sample_template):
        """テンプレート位置検出時の例外処理テスト"""
        mock_matchTemplate.side_effect = Exception("Position detection failed")
    ...
    @patch('src.utils.image_processing.cv2.matchTemplate')
    @patch('src.utils.image_processing.cv2.minMaxLoc')
    def test_logging_on_error_detection(self, mock_minMaxLoc, mock_matchTemplate, mock_logger, processor, sample_screenshot, error_templates):
        """エラー検出時のログ出力テスト"""
        mock_matchTemplate.return_value = np.array([[0.9]])
        mock_minMaxLoc.return_value = (0.1, 0.9, (10, 10), (20, 20))
    ...
    def test_detect_error_with_monitor_region(self, processor, sample_screenshot, error_templates):
        ...
        with patch('src.utils.image_processing.cv2.matchTemplate') as mock_match, \
             patch('src.utils.image_processing.cv2.minMaxLoc') as mock_minMaxLoc:
