...
logger.warning(f"設定ファイル読み込みエラー: {
    e
}")
...
logger.info(f"エラーテンプレート読み込み: {
    template_name
}")
except Exception as e:
logger.error(f"テンプレート読み込みエラー {
    filename
}: {
    e
}")
...
logger.info(f"復旧コマンド送信: {
    command
}")
return True

except Exception as e:
logger.error(f"復旧コマンド送信エラー: {
    e
}")
return False
...
chat_template_path = os.path.join(
    self.config['error_templates_dir'],
    'chat_input.png'
)
...
result = cv2.matchTemplate(screenshot, chat_template, cv2.TM_CCOEFF_NORMED)
min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
...
def save_error_template(self, template_name: str, region: Optional[Tuple[int, int, int, int]] = None):
"""
        現在の画面からエラーテンプレートを保存
        Args:
